...
class MobileNetV2():
    ...
    def build(self, x):
        with tf.variable_scope('stage0'):
            self.stage0 = conv_bn(x, self.depth(32), 3, 2)
        with tf.variable_scope('stage1'):
            self.stage1 = InvertedResidual(self.stage0, self.depth(32), self.depth(16), 1, 1) # 1/2

        with tf.variable_scope('ir1'):
            ir1 = InvertedResidual(self.stage1, self.depth(16), self.depth(24), 2, 6)
        with tf.variable_scope('stage2'):
            self.stage2 = InvertedResidual(ir1, self.depth(24), self.depth(24), 1, 6)
        ...
        with tf.variable_scope('stage3'):
            self.stage3 = InvertedResidual(ir3, self.depth(32), self.depth(32), 1, 6)
        ...
        with tf.variable_scope('ir6'):
            ir6 = InvertedResidual(ir5, self.depth(64), self.depth(64), 1, 6)
        with tf.variable_scope('stage4'):
            self.stage4 = InvertedResidual(ir6, self.depth(64), self.depth(64), 1, 6)
        ...
        with tf.variable_scope('stage5'):
            self.stage5 = InvertedResidual(ir8, self.depth(96), self.depth(96), 1, 6)
        ...
        with tf.variable_scope('stage6'):
            self.stage6 = InvertedResidual(ir10, self.depth(160), self.depth(160), 1, 6)
        
        with tf.variable_scope('stage7'):
            self.stage7 = InvertedResidual(self.stage6, self.depth(160), self.depth(320), 1, 6) # 1/32

        with tf.variable_scope('transit1'):
            self.transit1 = ResidualBlock(self.stage7, self.depth(320), self.depth(96))
        self.deconv1 = tf.layers.conv2d_transpose(self.transit1, self.depth(96), 4, strides=2, padding='same',
                                                  use_bias=False, name='deconv1')

        with tf.variable_scope('transit2'):
            self.transit2 = ResidualBlock((self.stage5+self.deconv1), self.depth(96), self.depth(32))
        self.deconv2 = tf.layers.conv2d_transpose(self.transit2, self.depth(32), 4, strides=2, padding='same',
                                                  use_bias=False, name='deconv2')

        with tf.variable_scope('transit3'):
            self.transit3 = ResidualBlock((self.stage3+self.deconv2), self.depth(32), self.depth(24))
        self.deconv3 = tf.layers.conv2d_transpose(self.transit3, self.depth(24), 4, strides=2, padding='same',
                                                  use_bias=False, name='deconv3')

        with tf.variable_scope('transit4'):
            self.transit4 = ResidualBlock((self.stage2+self.deconv3), self.depth(24), self.depth(16))
        self.deconv4 = tf.layers.conv2d_transpose(self.transit4, self.depth(16), 4, strides=2, padding='same',
                                                  use_bias=False, name='deconv4')

        with tf.variable_scope('transit5'):
            self.transit5 = ResidualBlock(self.deconv4, self.depth(16), self.depth(8))
        self.deconv5 = tf.layers.conv2d_transpose(self.transit5, self.depth(8), 4, strides=2, padding='same',
